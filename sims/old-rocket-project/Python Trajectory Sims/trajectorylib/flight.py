...
class Flight:
    ...
    def __init__(self, x0, v0, mass, tspan, dt, rocket: Rocket, launch_site_alt = 0, central_body = EARTH):
        """
        tspan (seconds)
        dt (seconds)
        """
    ...
    def initialize(self, x0, v0, q0, omega0, mass0, initial_altitude):
        ...
        # Initializes mass, 
        self.state[0,:] = x0.tolist() + v0.tolist() + q0.tolist() + omega0.tolist() + [mass0]
        self.altitude[0] = 0
    ...
    def propogate(self,integrator = 'lsoda', rail_constraints = True):
        ...
        self.pitch = np.array([np.arccos(unit(v)[2]) for v in self.rocket_axis]) * RAD2DEG
        # print(self.pitch)
    ...
    def general_flight(self, t, state):
        ...
        #----- Base Wind model -----#
        lateral_wind, pressure_deriv = self.base_wind.updateBase(altitude, rho, vz, 
        ...
        if in_recovery:
            ...
            Cd, Cd_friction, Cd_base, Cd_induced = CD_vals = self.rocket.get_Cd(self.rocket, mach, v_sound, angle_of_attack)
            # Cd = 0.46
            # Cd *= 1.1
            # if t > 15:
            #     print(CD_vals)
        drag_accel = - 0.5 * rho * v_rocket_wind * norm(v_rocket_wind) * Cd * self.rocket.A_cs / curr_mass
    ...
    def plot_state_vector(self, title = 'State Vector', figsize = (20,10), time_unit = 'second', length_unit = 'meter'):
    ...
    def plot_3d(self, title = 'Trajectory', figsize = (20,10), time_unit = 'second', length_unit = 'meter'):
    ...
    def plot_vectors(self, step, title = 'Trajectory', figsize = (20,10), length_unit = 'meter'):
