...
from trajectorylib import flight
from trajectorylib.math_helpers import quat
...
# Data
imu_accel_data = np.array(flight_data[[ICM_ACCEL_X_COL, ICM_ACCEL_Y_COL, ICM_ACCEL_Z_COL]])
imu_gyro_data = np.array(flight_data[[ICM_GYRO_X_COL, ICM_GYRO_Y_COL, ICM_GYRO_Z_COL]]) # * DEG2RAD
...
time_offset = time_data_s[491] -  time_data_s[490] + (time_data_s[490] -  time_data_s[489])
print(time_offset)
...
for i in range(step,DATA_LENGTH,step):
    ...
    a_launch[launch_i] = quat_apply(q_B2L, (a_body[i] + a_body[i-1])/2) - np.array([0,0, GRAVITY])
    # print(q_B2L)
    # print(a_launch[i])
    ...
    x_launch[launch_i] = v_launch[launch_i] * dt + 1/2 * a_launch[launch_i] * dt * dt + x_launch[launch_i-1]
    q_dot_launch[launch_i] = quat_mult((w_launch[launch_i] + w_launch[launch_i-1])/2, q_B2L) / 2.0
