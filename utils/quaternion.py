...
def quat_mult(q1: list | np.ndarray, q2: list | np.ndarray):
...
def quat_apply(quat: np.ndarray, vector: np.ndarray, rotation_type = "passive") -> np.ndarray:
    ...
    temp = quat_mult(quat, [0, *list(vector)])
    rslt = quat_mult(temp, conj(quat))
