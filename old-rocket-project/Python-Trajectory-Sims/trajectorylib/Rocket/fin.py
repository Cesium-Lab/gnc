...
class Fin(Component):
    ...
    def __init__(
        ...
        self.tip_top_pos = top_pos + np.array(
            [
                rocket_diam / 2 * np.cos(self.roll_orientation),
                rocket_diam / 2 * np.sin(self.roll_orientation),
                0,
            ]
        )
        ...
        CG_loc = self.tip_top_pos + np.array(
            [r * np.cos(self.roll_orientation), r * np.sin(self.roll_orientation), z]
        )
        ...
        super().__init__(
            mass=mass, top_pos=top_pos, CG_loc=CG_loc, I_matrix=0, name=name, type="Fin"
        )
        ...
        self.get_2d_points()
        self.set_plot_elements(fmt="k:", name=self.name)
    ...
    def _I_matrix_calc(self):
        ...
        self.Ixx = self.Iyy = (
            1 / 12 * self.mass * (3 * (r_o**2 + r_i**2) + length * length)
        )  # kg.m2
    ...
    def get_2d_points(self):
        ...
        if Cr > l_bt:
            ...
            print("tail longer")
            e = (
                np.array([0, (Cr - l_bt) * (r_rocket - r_tail) / l_bt + r_rocket])
                + tip_location
            )

        self.points_2d = np.array([a, b, c, d, e, a]) * np.array(
            [1, np.cos(self.roll_orientation)]
        )
