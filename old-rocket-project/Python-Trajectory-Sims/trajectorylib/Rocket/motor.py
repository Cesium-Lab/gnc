...
import pandas as pd
from ..constants import LBF2N, N2LBF
...
class Motor:
    ...
    def thrust_from_step_throttle(self, thrust, time, thrust_units="lbf"):
        ...
        self.thrust = (
            np.array([thrust[0], thrust[0], thrust[1], thrust[1]]) * unit_conversion
        )
        self.tb = max(time)
    ...
    def thrust_from_csv(
        self, filepath, delimeter=",", column_names=None, thrust_units=None
    ):
