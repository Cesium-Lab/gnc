...
class Fluid(Component):
    def __init__(
        ...
        # length is unknown
            ...
            raise ValueError(
                "Must define a fluid with three of four of [volume, length, density, diameter, wet_mass]"
            )
    ...
    def _I_matrix_calc(self):
        ...
        self.Ixx = self.Iyy = (
            1 / 12 * self.mass * (3 * (r**2) + length * length)
        )  # kg.m2
    ...
    def get_2d_points(self, t=0):
        ...
        # Y axis is radial
        r = self.diameter / 2.0
        bottom_x, bottom_y, bottom_z = self.top_pos + np.array(
            [0, 0, self.length_initial]
        )
    ...
    def update(self, t):
        ...
        self.CG_loc = self.CG_loc_dry + np.array(
            [0, 0, (self.length_initial - self.length) / 2]
        )
