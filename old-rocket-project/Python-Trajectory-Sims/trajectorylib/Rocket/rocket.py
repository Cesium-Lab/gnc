...
from .fin import Fin
from .bodytube import Bodytube
from .boattail import Boattail
from .motor import Motor
from ..constants import KG2LBM
...
class Rocket:
    ...
    def init_drag(self, k_roughness=0.00025, nu=1.81 * 10 ** (-5)):
        ...
        self.drag_0AoA = Drag0AoAModel(
            self.max_body_diameter, self.length, self.wetted_area, self.boattail.D_aft
        )

        self.induced_drag = InducedDragModel(
            self.length, self.nosecone.length, self.max_body_diameter
        )
    ...
    def calc_drag(self, rocket, mach, v_sound, aoa):
        skin_fric_body = self.drag_0AoA.skin_fric_coeff(
            self.length, self.drag_k_roughness, mach, v_sound, self.default_nu
        )
        skin_fric_fins = self.drag_0AoA.skin_fric_coeff(
            self.fins[0].Cr, self.drag_k_roughness, mach, v_sound, self.default_nu
        )
    ...
    def get_I_matrix(self, t, updated=False):
        ...
        for idx, f in enumerate(self.fluids):
            ...
            I_rel = I_matrix + f.mass * (
                np.inner(r, r) * np.identity(3) - np.outer(r, r)
            )
