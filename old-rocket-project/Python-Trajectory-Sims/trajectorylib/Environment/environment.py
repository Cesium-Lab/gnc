import numpy as np
from ..constants import DEG2RAD, FT2M, DAY2SEC
...
class WindModel:
    def __init__(
        ...
        self.u_star0 = (
            self.launch_site_wind_speed * self.k / np.log(launch_rail_height / self.z0)
        )
    ...
    def updateBase(
        self, alt, rho, vert_vel, prev_pressure_derivative, P_current=None, P_last=None
    ):
        """alt, rho --> (v_wind, dPdX)"""
        ...
        if alt == 0:
            ...
            wind_speed = (
                ...
                * (
                    np.log(alt / self.z0)
                    + alt / self.l_MBL
                    - alt / self.abl_height * (alt / 2 / self.l_MBL)
                )
            )
            ...
            for i, wind in enumerate(self.launch_site_wind_speed):
                ...
                if wind == 0:
                    ...
                    pressure_derivative[i] = (
                        prev_pressure_derivative[i] * (P_current / (P_last) + 1) / 2
                    )
...
class Drag0AoAModel:
    def __init__(self, max_body_diameter, body_length, body_wetted_area, boattail_diam):
    ...
    def skin_fric_coeff(
        self, length_c, k_roughness, Mach, v_sound, kinematic_viscosity=0
    ):
    ...
    def friction_drag_coeff(self, component, Cf, params_dict={}):
        ...
        if component == "body":
            ...
            a = (
                1
                + 60 / pow(body_length / max_diam, 3)
                + 0.0025 * (body_length / max_diam)
            )
    ...
    def base_drag_coeff(self, mach, Cd_friction_total):
        ...
        if mach < 0.6:
            Cd_b = (
                Kb
                * pow(self.boattail_diam / self.max_body_diameter, n)
                / np.sqrt(Cd_friction_total)
            )
...
class InducedDragModel:
    def __init__(self, rocket_length, nosecone_length, diameter):
        ...
        self.delta = lambda aoa: np.interp(
            aoa, self.interp_table["aoa"], self.interp_table["delta"]
        )
        self.eta = lambda aoa: np.interp(
            aoa, self.interp_table["aoa"], self.interp_table["eta"]
        )
    ...
    def induced_drag(self, aoa):
        ...
        b = (
            3.6
            * eta
            * (1.36 * self.rocket_length - 0.55 * self.nosecone_length)
            / (np.pi * self.diameter)
        )
        Cd_body = a + b * aoa**3
