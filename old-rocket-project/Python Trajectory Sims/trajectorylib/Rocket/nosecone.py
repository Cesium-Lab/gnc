...
class Nosecone(Component):
    ...
    def __init__(self, length, shape, D_o, thickness, shoulder_diameter = None, shoulder_length = None, parabolic_K = None, density = None, name = "Nosecone", mass = None):
        ...
        super().__init__(mass=self.mass, top_pos=0, CG_loc = self.CG_loc, I_matrix=self.I_matrix, name = name, type = "Nosecone")
    ...
    def _I_matrix_calc(self):
        ...
        self.Ixx = self.Iyy = 1/10 * m * l**2 + 3/20 * m * r**2
        self.Izz = 3/10 * m * r**2
    ...
    def _attributes_based_on_shape(self, shape): # TODO
        ...
        if shape == "ogive":
            ...
            if not parabolic_K or parabolic_K < 0 or parabolic_K > 1:
                raise ValueError("Parabolic nosecone requires K between 0 and 1 inclusive")
            
            self.func = lambda x: r * (2*x/length - parabolic_K * (x/length)**2) / (2 - parabolic_K)
            ...
            raise ValueError("Shape must be 'ogive', 'conical', or 'parabolic'. Other shapes not supported yet")
