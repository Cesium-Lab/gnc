...
class Boattail(Component):
    ...
    def __init__(self, top_pos, length, D_front, D_aft, thickness, density = None, angle = None, name = "Boattail", mass = None):
        ...
        if not all([D_front, D_aft, top_pos, length, thickness]):
            raise ValueError('Must define a bodytube with "D_front", "D_aft", "top_pos", "length", and "thickness"')
        ...
        super().__init__(mass=self.mass, top_pos=top_pos, CG_loc = self.CG_loc, I_matrix=self.I_matrix, name = name, type = "Boattail")
